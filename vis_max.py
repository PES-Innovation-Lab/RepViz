...
def create_scatter_plot(x_param, fixed_params):
    ...
    # Group by the x_param and calculate max for REPCL_SIZE and VECCL_SIZE
    grouped = filtered_df.groupby(x_param)[["REPCL_SIZE", "VECCL_SIZE"]].max().reset_index()
    ...
    ax.set_ylabel("Size")
    ax.set_title(f"Max REPCL_SIZE and VECCL_SIZE vs {x_param}\n" + 
                 ", ".join([f"{k}={v}" for k, v in fixed_params.items()]))
    ax.legend()
    
    # Create directory for saving plots if it doesn't exist
    os.makedirs("scatter_plots_max", exist_ok=True)
    
    # Generate filename
    filename = f"scatter_max_{x_param}_" + "_".join([f"{k}{v}" for k, v in fixed_params.items()]) + ".png"
    plt.savefig(os.path.join("scatter_plots_max", filename))
    plt.close()
...
print("All scatter plots with maximum values have been generated and saved in the 'scatter_plots_max' directory.")
